    ...
    queryFn: async () => {
      ...
      recurringData?.forEach(schedule => {
        if (schedule.shift_types?.name && schedule.profiles) {
          ...
          // Skip if already processed or officer has full-day PTO or is Probationary
          if (processedOfficers.has(officerKey) || hasFullDayPTO(officerId, shiftTypeId) || isProbationary(officerRank)) {
            return;
          }
      ...
      workingExceptions?.forEach(exception => {
        if (exception.shift_types?.name && exception.profiles) {
          ...
          // Skip if already processed (from recurring) or officer has full-day PTO or is Probationary
          if (processedOfficers.has(officerKey) || hasFullDayPTO(officerId, shiftTypeId) || isProbationary(officerRank)) {
            return;
          }
