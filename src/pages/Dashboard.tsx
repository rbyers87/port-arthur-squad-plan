...
const Dashboard = () => {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Get ALL schedule exceptions for today (both working and PTO)
      const { data: allExceptionsData, error: exceptionsError } = await supabase
      ...
      // Separate working exceptions from PTO exceptions
      const workingExceptions = allExceptionsData?.filter(e => !e.is_off) || [];
      const ptoExceptions = allExceptionsData?.filter(e => e.is_off) || [];
      ...
      // Helper function to check if officer has full-day PTO (matches DailyScheduleView logic)
      const hasFullDayPTO = (officerId: string, shiftTypeId: string) => {
        if (!ptoExceptions) return false;
        
        const ptoException = ptoExceptions.find(pto => 
      ...
      recurringData?.forEach(schedule => {
        if (schedule.shift_types?.name && schedule.profiles) {
          ...
          const position = schedule.position_name;
          
          const officerKey = `${officerId}-${shiftTypeId}`;
          
          // Skip if already processed, officer has full-day PTO, is Probationary, or is in special assignment
          if (processedOfficers.has(officerKey) || 
          ...
          // Mark as processed
          processedOfficers.add(officerKey);
      ...
      workingExceptions?.forEach(exception => {
        if (exception.shift_types?.name && exception.profiles) {
          ...
          const position = exception.position_name;
          
          const officerKey = `${officerId}-${shiftTypeId}`;
          
          // Skip if already processed (from recurring), officer has full-day PTO, is Probationary, or is in special assignment
          if (processedOfficers.has(officerKey) || 
              hasFullDayPTO(officerId, shiftTypeId) || 
              isProbationary(officerRank) ||
              isSpecialAssignment(position)) {
      ...
      console.log("📊 Dashboard Staffing Counts:", {
        processedOfficers: Array.from(processedOfficers),
